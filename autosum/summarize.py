...
def summarize_text(summarizer, text, split_size=300, min_tokens=200,
                   max_context=None, max_summary_size=1000):
    ...
    if ntokens > max_context:  # If we can't fit in context split into smaller chunks
        ...
        text = text.split()
        samples = [summarize_text(summarizer, ' '.join(text[i:i+split_size]),
                                  split_size-10)
...
def main():
    ...
    LOGGER.info(
        f"Original -> lines: {len(doc_lines)} words: {len(document.split(' '))} chars {len(document)}")

    summary = summarize_text(summarizer, document,
                             max_summary_size=args.max_size)
